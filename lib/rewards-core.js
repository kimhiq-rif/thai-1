...
(function (root, factory) {
  ...
  //   requiredLevel   — the level this challenge runs at. A fixed value (sprint
  //                     = '3') locks it; null means the learner picks the level
  //                     when starting, and it is stored on the live snapshot.
  //                     Either way, only answers from that level count.
  //   requiredLevel12 — minimum Level-1.2 answers required (0 = off). Retired
  //                     for the load challenge in v1.25.39: it used to force the
  //                     first 10 questions to Level 1.2, which overrode the
  //                     learner's chosen level. The mechanism stays supported.
  const CHALLENGES = {
    load:   { id: 'load',   target: 50, requiredAccuracy: 0.7, durationMs: 2 * 60 * 60 * 1000, reward: 25, requiredLevel12: 0, requiredLevel: null },
  ...
  // --- Mystery box (M5) ---------------------------------------------------
  // Bounded, guaranteed-floor variable reward: every outcome is a real prize
  // (never nothing). `rand` is an injectable () => [0,1) for testability.
  const MYSTERY_POOL = [
    { weight: 40, reward: { type: 'points', amount: 5 } },
    { weight: 25, reward: { type: 'points', amount: 10 } },
    { weight: 12, reward: { type: 'points', amount: 20 } },
    { weight: 12, reward: { type: 'token', token: 'hint' } },
    { weight: 7,  reward: { type: 'token', token: 'freeze' } },
    { weight: 4,  reward: { type: 'token', token: 'boost' } },
  ];
  function openMysteryBox(rand){
    const r = (typeof rand === 'function' ? rand() : Math.random());
    const total = MYSTERY_POOL.reduce((s, e) => s + e.weight, 0);
    let x = Math.max(0, Math.min(0.999999, r)) * total;
    for(const e of MYSTERY_POOL){
      if(x < e.weight) return e.reward;
      x -= e.weight;
    }
    return MYSTERY_POOL[0].reward; // floor: always a real reward
  }

  // --- Character dex (M5) -------------------------------------------------
  // Mastery status of a single item from its {correct,wrong} stat.
  function dexItemStatus(stat, threshold){
    const th = (threshold == null) ? 3 : threshold;
    if(!stat) return 'new';
    const c = Number(stat.correct) || 0, w = Number(stat.wrong) || 0;
    if(c + w === 0) return 'new';
    return (c - w * 1.5) >= th ? 'mastered' : 'seen';
  }
  function dexProgress(itemStats, ids, threshold){
    let mastered = 0;
    for(const id of ids){ if(dexItemStatus(itemStats && itemStats[id], threshold) === 'mastered') mastered++; }
    return { mastered, total: ids.length };
  }

  return { hasTierUnlock, TIER, defaultTokens, mergeTokens, computeChallengeTokens, addTokens, CHALLENGES, answerCountsToward, isChallengeWon, inkScore, inkVerdict, dilate, openMysteryBox, dexItemStatus, dexProgress };
});
