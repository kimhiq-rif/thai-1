...
window.SkinsFX = (function(){
  ...
  var ambientRoot=null, ambientCanvas=null, ambientCtx=null, ambientRaf=null, ambientTheme=null, ambientDots=[];
  ...
  function ambient(themeId){
    ...
    ambientDots=Array.from({length:themeId==='master'?28:20},function(){return{x:Math.random()*innerWidth,y:Math.random()*innerHeight,r:1+Math.random()*3,v:.12+Math.random()*.32};});
    ambientTick();
  }

  function ambientTick(){ if(!ambientCtx||document.hidden){ambientRaf=null;return;} ambientCtx.clearRect(0,0,innerWidth,innerHeight);
    ambientDots.forEach(function(p){p.y+=(ambientTheme==='coral'?-p.v:p.v);if(p.y<0)p.y=innerHeight;if(p.y>innerHeight)p.y=0;
  ...
  function stop(){
    stopEntrance(); if(ambientRaf){cancelAnimationFrame(ambientRaf);ambientRaf=null;}if(ambientRoot){ambientRoot.remove();ambientRoot=null;ambientCanvas=null;ambientCtx=null;}ambientDots=[];ambientTheme=null;
  }

  document.addEventListener('visibilitychange', function(){
    if(document.hidden && raf){ cancelAnimationFrame(raf); raf = null; }
    else if(!document.hidden && particles.length && !raf) raf = requestAnimationFrame(tick);
    if(document.hidden&&ambientRaf){cancelAnimationFrame(ambientRaf);ambientRaf=null;}else if(!document.hidden&&ambientCtx&&!ambientRaf)ambientRaf=requestAnimationFrame(ambientTick);
  });
