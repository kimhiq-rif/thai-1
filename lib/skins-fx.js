...
window.SkinsFX = (function(){
  ...
  function seedParticles(kind){
    ...
    var profile = kind === 'cyber'
      ...
      : kind === 'midnight'
        ...
        : kind === 'lotus'
          ? {colors:['#f0abfc','#e879f9','#f9a8d4','#fef3c7'],glow:'#e879f9',count:window.innerWidth<700?30:44}
  ...
  function tick(){
    ...
    for(var i=particles.length-1; i>=0; i--){
      ...
      if(p.kind === 'cyber') ctx.fillRect(p.x-p.r*2,p.y-p.r/2,p.r*4,p.r);
        ...
        ctx.ellipse(0,0,p.r*1.15,p.r*2.5,0,0,Math.PI*2);ctx.fill();ctx.restore();
      }
  ...
  function ambient(themeId){
    if(!/^(lotus|coral|festival|master)$/.test(themeId)||reduced()) return;
  ...
  function onAmbientScroll(){
    if(!ambientRoot||ambientTheme!=='lotus'||reduced()||scrollRaf) return;
    scrollRaf=requestAnimationFrame(function(){
      scrollRaf=null;if(!ambientRoot)return;
      var y=window.scrollY||0;
      ambientRoot.style.setProperty('--skin-scroll-far',(Math.sin(y/720)*6).toFixed(2)+'px');
      ambientRoot.style.setProperty('--skin-scroll-near',(Math.sin(y/430)*11).toFixed(2)+'px');
    });
  }
  ...
  function ambientTick(now){
    ...
    ambientDots.forEach(function(p){p.y+=(ambientTheme==='coral'||ambientTheme==='lotus'?-p.v:p.v)*step;if(p.y<0)p.y=window.innerHeight;if(p.y>window.innerHeight)p.y=0;
