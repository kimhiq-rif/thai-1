...
window.Juice = (function(){
  ...
  function chimeCorrect(opts){
    if(!ensure()) return;
    if(opts&&opts.sound==='island'){
      note(392,0,.28,.06,'sine');note(493.88,.08,.32,.055,'triangle');note(587.33,.17,.4,.045,'sine');note(783.99,.28,.45,.03,'sine');return;
    }
    ...
    if(opts&&opts.sound==='sakura'){
      note(440,0,.22,.052,'triangle');note(523.25,.1,.3,.048,'sine');note(659.25,.22,.42,.038,'triangle');return;
    }
    if(opts&&opts.sound==='mango'){
      note(523.25,0,.16,.065,'triangle');note(659.25,.07,.18,.06,'triangle');note(783.99,.14,.22,.052,'sine');note(1046.5,.23,.28,.035,'triangle');return;
    }
    if(opts&&opts.sound==='rainforest'){
      note(293.66,0,.3,.058,'triangle');note(440,.09,.36,.052,'sine');note(587.33,.2,.46,.04,'sine');return;
    }
  ...
  function chimeWin(opts){ if(!ensure()) return;
    if(opts&&opts.sound==='island'){
      [392,493.88,587.33,783.99,987.77].forEach(function(f,i){note(f,i*.09,.58,.09,'sine');});return;
    }
    if(opts&&opts.sound==='lotus'){
    ...
    if(opts&&opts.sound==='sakura'){
      [440,523.25,659.25,880].forEach(function(f,i){note(f,i*.12,.65,.08,i%2?'sine':'triangle');});note(329.63,0,.9,.025,'sine');return;
    }
    if(opts&&opts.sound==='mango'){
      [523.25,659.25,783.99,1046.5,1318.5].forEach(function(f,i){note(f,i*.075,.34,.095,'triangle');});return;
    }
    if(opts&&opts.sound==='rainforest'){
      [293.66,392,440,587.33,783.99].forEach(function(f,i){note(f,i*.11,.62,.075,i<2?'triangle':'sine');});return;
    }
  ...
  function islandDrops(cx,cy,n,pal,power){for(var i=0;i<n;i++){var a=Math.random()*Math.PI*2,sp=(1.8+Math.random()*5)*power;
    push({t:'island',x:cx,y:cy,vx:Math.cos(a)*sp,vy:Math.sin(a)*sp-1.8,life:1,decay:.013+Math.random()*.016,size:3+Math.random()*5,col:pal[(Math.random()*pal.length)|0]});}}
  function sakuraPetals(cx,cy,n,pal,power){for(var i=0;i<n;i++){var a=Math.random()*Math.PI*2,sp=(1.4+Math.random()*4.5)*power;
    push({t:'sakura',x:cx,y:cy,vx:Math.cos(a)*sp,vy:Math.sin(a)*sp-2,life:1,decay:.01+Math.random()*.014,w:4+Math.random()*5,h:7+Math.random()*8,rot:Math.random()*6,vr:(Math.random()-.5)*.28,col:pal[(Math.random()*pal.length)|0]});}}
  function mangoGems(cx,cy,n,pal,power){for(var i=0;i<n;i++){var a=Math.random()*Math.PI*2,sp=(2+Math.random()*5.5)*power;
    push({t:'mango',x:cx,y:cy,vx:Math.cos(a)*sp,vy:Math.sin(a)*sp-2.4,life:1,decay:.014+Math.random()*.018,size:3+Math.random()*5,rot:Math.random()*6,vr:(Math.random()-.5)*.35,col:pal[(Math.random()*pal.length)|0]});}}
  function rainforestLeaves(cx,cy,n,pal,power){for(var i=0;i<n;i++){var a=Math.random()*Math.PI*2,sp=(1.6+Math.random()*4.8)*power;
    push({t:'rainforest',x:cx,y:cy,vx:Math.cos(a)*sp,vy:Math.sin(a)*sp-1.7,life:1,decay:.011+Math.random()*.014,w:3+Math.random()*5,h:9+Math.random()*9,rot:Math.random()*6,vr:(Math.random()-.5)*.2,col:pal[(Math.random()*pal.length)|0]});}}
  ...
  function tick(){ ctx.clearRect(0,0,canvas.width,canvas.height);
    for(var i=parts.length-1;i>=0;i--){ var p=parts[i]; p.vy+=0.14; p.x+=p.vx; p.y+=p.vy; p.vx*=0.99; p.life-=p.decay;
      ...
      else if(p.t==='island'){ctx.strokeStyle=p.col;ctx.lineWidth=1.6;ctx.shadowColor=p.col;ctx.shadowBlur=7;ctx.beginPath();ctx.arc(p.x,p.y,p.size,0,Math.PI*2);ctx.stroke();ctx.shadowBlur=0;}
      else if(p.t==='sakura'){p.rot+=p.vr;ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.rot);ctx.fillStyle=p.col;ctx.beginPath();ctx.ellipse(0,0,p.w,p.h,0,0,Math.PI*2);ctx.fill();ctx.restore();}
      else if(p.t==='mango'){p.rot+=p.vr;ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.rot);ctx.fillStyle=p.col;ctx.shadowColor=p.col;ctx.shadowBlur=8;ctx.fillRect(-p.size,-p.size,p.size*2,p.size*2);ctx.restore();}
      else if(p.t==='rainforest'){p.rot+=p.vr;ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.rot);ctx.fillStyle=p.col;ctx.beginPath();ctx.moveTo(0,-p.h);ctx.quadraticCurveTo(p.w,0,0,p.h);ctx.quadraticCurveTo(-p.w,0,0,-p.h);ctx.fill();ctx.restore();}
  ...
  function correct(el,opts){ if(!root) return; chimeCorrect(opts); haptic(false);
    ...
    if(opts&&opts.id==='island'){islandDrops(c.x,c.y-6,20,pal,1);sparks(c.x,c.y-6,7,pal,.65);}
    else if(opts&&opts.id==='lotus'){lotusPetals(c.x,c.y-6,22,pal,1);sparks(c.x,c.y-6,9,pal,.72);}
    else if(opts&&opts.id==='sakura'){sakuraPetals(c.x,c.y-6,20,pal,1);sparks(c.x,c.y-6,6,pal,.55);}
    else if(opts&&opts.id==='mango'){mangoGems(c.x,c.y-6,18,pal,1);sparks(c.x,c.y-6,10,pal,.75);}
    else if(opts&&opts.id==='rainforest'){rainforestLeaves(c.x,c.y-6,20,pal,1);sparks(c.x,c.y-6,7,pal,.6);}
  ...
  function win(opts){ if(!root) return; chimeWin(opts); haptic(true);
    ...
    var glow=id==='island'?'rgba(13,148,136,.46)':id==='lotus'?'rgba(192,38,211,.48)':id==='sakura'?'rgba(190,24,93,.43)':id==='mango'?'rgba(245,158,11,.46)':id==='rainforest'?'rgba(22,163,74,.46)':royal?'rgba(127,29,29,.52)':id==='cyber'?'rgba(6,182,212,.48)':id==='midnight'?'rgba(79,70,229,.5)':'rgba(224,169,59,.45)';
    ...
    if(id==='island') islandDrops(cx,cy,54,pal,1.2);
    else if(id==='lotus') lotusPetals(cx,cy,58,pal,1.18);
    else if(id==='sakura') sakuraPetals(cx,cy,58,pal,1.15);
    else if(id==='mango') mangoGems(cx,cy,52,pal,1.2);
    else if(id==='rainforest') rainforestLeaves(cx,cy,56,pal,1.16);
    ...
    setTimeout(function(){ if(id==='island') islandDrops(cx,cy-30,26,pal,1.08); else if(id==='lotus') lotusPetals(cx,cy-30,28,pal,1.08); else if(id==='sakura') sakuraPetals(cx,cy-30,28,pal,1.04); else if(id==='mango') mangoGems(cx,cy-30,24,pal,1.1); else if(id==='rainforest') rainforestLeaves(cx,cy-30,28,pal,1.05); else if(royal) royalLeaves(cx,cy-30,24,pal,1.15); else if(id==='cyber') cyberBits(cx,cy-30,30,pal,1.1); else if(id==='midnight') comets(cx,cy-30,20,pal,1); else if(/^(coral|festival|master)$/.test(id)) motifs(id,cx,cy-30,30,pal,1.05); else confetti(cx,cy-30,22,GOLD,1.1); },180);
