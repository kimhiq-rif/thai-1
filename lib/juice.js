...
window.Juice = (function(){
  ...
  function correct(el,opts){ if(!root) return; chimeCorrect(opts); haptic(false);
    ...
    if(opts&&opts.id==='royal'){ royalLeaves(c.x,c.y-6,16,pal,1); royalCrowns(c.x,c.y-8,5,pal); sparks(c.x,c.y-6,10,pal,1); }
  ...
  function win(opts){ if(!root) return; chimeWin(); haptic(true);
    if(reduced()){
    ...
    var glow=royal?'rgba(127,29,29,.52)':id==='cyber'?'rgba(6,182,212,.48)':id==='midnight'?'rgba(79,70,229,.5)':'rgba(224,169,59,.45)';
    ...
    if(royal){ royalLeaves(cx,cy,46,pal,1.15); royalCrowns(cx,cy,12,pal); }
    ...
    setTimeout(function(){ if(royal) royalLeaves(cx,cy-30,24,pal,1.15); else if(id==='cyber') cyberBits(cx,cy-30,30,pal,1.1); else if(id==='midnight') comets(cx,cy-30,20,pal,1); else if(/^(coral|festival|master)$/.test(id)) motifs(id,cx,cy-30,30,pal,1.05); else confetti(cx,cy-30,22,GOLD,1.1); },180);
